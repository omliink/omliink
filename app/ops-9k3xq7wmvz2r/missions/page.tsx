...
import AdminMissionsList from '@/components/admin/AdminMissionsList'
...
export default async function AdminMissionsPage() {
  ...
  return (
    <div>
      ...
      <div className="mt-10">
        <h2 className="text-lg font-semibold text-gray-900">Toutes les missions</h2>
        <p className="mt-1 text-sm text-gray-600">{missions.length} mission(s) au total, tous statuts confondus.</p>

        {missions.length === 0 ? (
          <div className="mt-4">
            <EmptyState title="Aucune mission" description="Rien à modérer pour le moment." />
          </div>
        ) : (
          <AdminMissionsList missions={missions} categoryNameById={categoryNameById} employerNameById={employerNameById} />
        )}
      </div>
    </div>
  )
